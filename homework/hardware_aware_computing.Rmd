...
require(tidyverse)
require(ggbeeswarm) # for `autplot`ting `bench` output
require(cowplot)
...
ar_precision_matvec_R(1:10, auto_corr)
```

```{r}
ar_precision_matvec(1:10, auto_corr)
```

```{r}
ar_precision_matvec_R(1:10, auto_corr) - ar_precision_matvec(1:10, auto_corr)
```
...
benchmark_matvec <- 
  bench::mark(
    ar_precision_matvec_R(x, auto_corr),
    ar_precision_matvec(x, auto_corr)
  )
...
```{r}
autoplot(benchmark_matvec)
```

> Looks like the Rccp implementation of AR(1) matrix-vector multiplication is ~9 times faster.
...
ar_eig_direct <-
  function() {
    ...
    ar_eigs <-
      eigen(ar_cov_matrix, symmetric = TRUE)
  }

# Also try using the RSpectra package without
# our hand-rolled multiplication
...
> It takes about 2/3rds as long. But it also looks like `RSpectra` with an explicit matrix is a bit more efficient than our manual `Rcpp` matrix-vector multiplication.
...
> They're each ~150-200x times slower than `%*%`/BLAS. Col oriented takes ~75% the time of row oriented.
> BLAS info:
```
Matrix products: default
BLAS/LAPACK: /nix/store/py9w4nc0zzhxawly2dfz65dksphgrys6-openblas-0.3.7/lib/libopenblasp-r0.3.7.so
```
...
matmat_benchmark <-
  bench::mark(
    ...
    row_col_dot_matmat(A, B),
    col_oriented_matmat(A, B),
    matmat_via_eigen(A, B),
    check = FALSE,
) %>%
  mutate(expression = as.character(expression))

autoplot(matmat_benchmark)
```

> Native wins on this benchmark—and by a longshot—with this size of matrix ...